...
import static com.purbon.kafka.topology.backend.RedisBackend.JULIE_OPS_BUILDER_BINDINGS;
import static com.purbon.kafka.topology.backend.RedisBackend.JULIE_OPS_BUILDER_TYPE;
...
public class RedisBackendTest {
  ...
  public void testSaveBindings() {
    ...
    when(jedis.sadd(eq(JULIE_OPS_BUILDER_BINDINGS), any())).thenReturn(1l);

    stateProcessor.saveBindings(new HashSet<>(Arrays.asList(binding)));

    verify(jedis, times(1)).sadd(eq(JULIE_OPS_BUILDER_BINDINGS), any());
  }
